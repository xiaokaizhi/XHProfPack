 ...
 * @date	2014-06-09 09:30:15
 */
class obj_time extends obj {
	 ...
	 *  根据时间计算 
	 */
	public function getHourSpan($count){
		$date = array();
		for($i = 0; $i <= $count; $i++){
			if($i == 0){
				$time = date('Y-m-d H');
				...
				//$date[$key] = array("{$time}:00:00", "{$currentTime}");
				$date[$key] = array(strtotime("{$time}:00:00"),time());
			}else {
				$time = date('Y-m-d H', strtotime("-{$i} hour",time()));
				$key = date('md-H', strtotime("-{$i} hour",time()));
				//$date[$key] = array("{$time}:00:00", "{$time}:59:59");
				$date[$key] = array(strtotime("{$time}:00:00"),strtotime("{$time}:59:59"));
			}
		}
		$result = array_reverse($date);
