...
$(function(){
    ...
    $('button').click(function(event){
        ...
        var gid = $("#"+btnid).attr("gid");
        gid = Number(gid);
        var subgroupname = $("#"+btnid).attr("subgroupname");
        subgroupname = $.trim(subgroupname);
        console.log('gid:',gid,'subgroupname:',subgroupname);

        //读取小组结果 //todo
        if(gid != '' && subgroupname !=''){
            $('#tonegroupid').empty().val(gid);
            $('#tonesubgroupname').empty().val(subgroupname);
            $.ajax({
                type: "POST",
                url: "evaluate?search=1",
                dataType: "html",
                data: {"type":type,"date":date,"gid":gid,"subgroupname":subgroupname,"r":Math.random()},
                success: function(string){
                    console.log('button-ajax返回结果：',string);
                    if(string != ""){
                        $("#editor").html(string);
                    }else {
                        $("#editor").empty();
    ...
    $("#btnsubmit").click(function(){ //ajax 将数据插入db  隐藏表单获取groupid 和subgroupname
        var date = $("#dp1").val();
        var type = $("#objtype").val();
        var gid = $('#tonegroupid').val();
        var subgroupname = $('#tonesubgroupname').val();
        var content = $('#editor').html();
        var content = encodeURIComponent(content);
        console.log('editor:',content);
        $.ajax({
            type: "POST",
            url: "evaluate?search=2",
            dataType: "html",
            data: {"type":type,"date":date,"gid":gid,"subgroupname":subgroupname,"content":content,"r":Math.random()},
            success: function(string){
                console.log('btnsubmit-ajax返回结果：',string);
                if(string != ""){
                    console.log('更新数据成功type_date_gid_subgroupname',type+'_'+date+'_'+gid+'_'+subgroupname);
                }else {
                    console.error('更新写入数据失败！');
                }
            }
        });
        $('#describeinfo').modal('hide');
    });
})
