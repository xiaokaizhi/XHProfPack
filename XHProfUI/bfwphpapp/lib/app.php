...
class BFW_App {
    ...
    public function run($classMethod = null){
    	try {
    		if ($this->_userError) set_error_handler(array('BFW_Error', 'setError'));
    		BFW_Debug::$debug = $this->_debug;
			...
			if (!$classMethod) $classMethod = BFW_Request::request($this->_target);
			if (!$classMethod) {
				$argv = BFW_Request::server('argv');
				if (!empty($argv[1])) {
					$classMethod = $argv[1];
					unset($argv[1]);
				}
			...
			$res = BFW_Controller::get($classMethod, $argv);
			if ($res) {
				print_r($res);
			}
    ...
    public function __destruct() {
    	try {
    		BFW_Debug::writeDebugInfo();// 将调试信息写入文件，在程序结束时调用
    		BFW_Log::write();// 写入日志，在程序结束时调用
    	} catch (Exception $e) {
    		$code= $e->getCode();
			$msg = $e->getMessage();
